        ...
        protected override void Verify(ApplicantProfilePoco[] pocos)
        {
             ...
             foreach(ApplicantProfilePoco poco in pocos)
            {
                if (poco.CurrentSalary < 0)
                    exceptions.Add(new ValidationException(111, $"{poco.Id} 's salary cannot be negative!!"));
                if(poco.CurrentRate<0)
                    exceptions.Add(new ValidationException(112, $"{poco.Id} 's current rate cannot be negative!!" ));
            }
